-- utworzenie schemy
create schema biblioteka2;
set search_path to biblioteka2;
...
-- SKRYPT WLASCIWY
create or replace function biblioteka2.getBooksFromReaderID(readerID integer)
    returns table
            (
                title    varchar(32),
                returned bool
...
$$
begin
    return query select k.tytul, (w.data_zwrotu is not null)
                 from biblioteka2.wypozyczenie w
                          join biblioteka2.wypozyczenie_ksiazki using (wypozyczenie_id)
                          join biblioteka2.ksiazka k using (ksiazka_id)
                 where czytelnik_id = readerID;
end;
$$ language plpgsql;

-- WSTAWIENIE DANYCH NA POTRZEBY TESTOWANIA
insert into biblioteka2.czytelnik (imie, nazwisko)
values ('Tomasz', 'Kowalski');

insert into biblioteka2.wypozyczenie (czytelnik_id, data_pozyczenia, data_zwrotu)
values (( select czytelnik_id from biblioteka2.czytelnik where imie = 'Tomasz' and nazwisko = 'Kowalski' ),
        current_date, null);

insert into biblioteka2.wypozyczenie_ksiazki (ksiazka_id, wypozyczenie_id)
values (4, ( select wypozyczenie_id
             from biblioteka2.wypozyczenie
             where czytelnik_id = ( select czytelnik_id
                                    from biblioteka2.czytelnik
                                    where imie = 'Tomasz' and nazwisko = 'Kowalski' ) ));